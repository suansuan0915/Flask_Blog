...
@app.route("/home")
def hello():
    page = request.args.get('page', 1, type=int)  # get next page
    posts = Post.query\
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Your account has been created. You are now to log in', category='success')
...
def post(post_id):
    post = Post.query.get_or_404(post_id) # if page not exists, return 404
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your password has been updated. You are now to log in', category='success')
        return redirect(url_for("login"))
